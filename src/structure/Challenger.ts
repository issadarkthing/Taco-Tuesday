...
export class Challenger extends Fighter {
  ...
  constructor(player: Player) {
    ...
    this.attack = player.attack + random.integer(-5, 4);
    this.hp = player.hp + random.integer(-6, 5);
    this.armor = player.armor + (random.integer(-4, 3) / 100);
    this.critChance = player.critChance + (random.integer(-5, 4) / 100);
    this.critDamage = player.critDamage + random.integer(-0.5, 0.4);
  }

  show() {
    const embed = super.show();
    embed.addField("Loot Drop", `${this.drop} ${currency}`, true);
    embed.addField("XP Drop", `${this.xpDrop} XP`, true);
